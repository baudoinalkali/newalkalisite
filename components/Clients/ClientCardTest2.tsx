...
const ClientCardTest = props => {
    const clientNameLength = () => {
        return `${props.name.length > 20 ? '20rem' : '22.4rem'} `
    }
    ...
    return (
        <a href={`/clients/${props.slug}`}>
            <div className="group duration-500 rounded shadow-2xl px-4 pb-10 m-auto client-card-bg" style={{ paddingTop: `${clientNameLength()}`, height: "450px", backgroundImage: `url("${props.backgroundImage}")`, backgroundSize: "cover", backgroundRepeat: "no-repeat", backgroundColor: `${props.backgroundColor}` }}>
