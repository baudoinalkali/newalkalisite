...
import clientContent from "../../content/clientPages/clientContent";
...
export default function ClientPage({clientContent}) {
  const ameliaGoalImage = clientContent.amelia.section1.map((section1) => (
    <StandardImage
      imageAngle="left"
  ...
  const ameliaGoalText = clientContent.amelia.section1.map((section1) => (
    <StandardTextBlock
      sub={section1.sub}
  ...
  const ameliaResultImage = clientContent.amelia.section2.map((section2) => (
    <StandardImage
      imageAngle="right"
  ...
  const ameliaResultText = clientContent.amelia.section2.map((section2) => (
    <StandardTextBlock
      sub={section2.sub}
  ...
  return (
    <div className="m-auto overflow-hidden">
      ...
      <div className="">
        {clientContent.amelia.hero.map((hero) => (
          <ClientHeader
            sub={hero.industry}
            title={hero.name}
      ...
      {clientContent.amelia.about.map((amelia) => (
        <ClientAbout
          name={amelia.name}
          about={amelia.bio}
          slug={amelia.slug}
      ...
      />
      {clientContent.amelia.about.map((about) => (
        <ClientScheduleCTA
          brandColor={about.brandColor}
        />
      ))}
      {clientContent.amelia.screens.map((screens) => (
        <ResponsiveScreens
